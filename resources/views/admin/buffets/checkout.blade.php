            ...
            <div class="tile-body">
                <div class="row">
                    <div class="col-md-8 text-center">
                        <form action="{{ route('admin.buffet.kot.search') }}" method="get" autocomplete="off">
                            @csrf
                            <div class="row mt-2">
                                ...
                                <div class="col-md-5 ml-0">
                                    <div class="input-group mb-3">
                                        <input type="text" class="form-control" placeholder="Search by Table No"
                                            name="search"
                                            value="{{ $order_id ? App\Models\Buffetorder::where('id', $order_id)->first()->order_tableNo : '' }}">
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                <table class="table table-bordered" id="tbl-sale-cart">
                                    ...
                                    @if($order_id && App\Models\Buffetorder::where('id',
                                    $order_id)->first()->status == 'receive')
                                    <tbody>
                                        @foreach( App\Models\Buffetsale::where('buffetorder_id',
                                ...
                                <div class="row">
                                    <div class="col-sm-12" style="visibility:{{ $order_id && App\Models\Buffetorder::where('id',
                        ...
                        <div class="border px-4 rounded pb-4 mb-4" style="border-color:rgb(182, 182, 182);">
                            ...
                            <div class="text-center">
                                ...
                                <button type="button" onclick="print();"
                                    ...
                                    {{$order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered' ? '' : 'disabled' }}>Print
                                    Receipt</button>
                            </div>
                        </div>

                        <!--end of pos print using javascript-->
                        <form method="POST" action="{{ route('admin.buffet.kot.orderupdate') }}">
                            @csrf
                            ...
                            <div class="border px-4 rounded" style="border-color:rgb(182, 182, 182);">
                                ...
                                <div class="form-group mt-2 mb-4">
                                    <button type="submit" class="btn btn-primary text-uppercase"
                                        style="display:block; width:100%;"
                                        {{ $order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'receive' ? '' : 'disabled' }}
  ...
  JSPM.JSPrintManager.auto_reconnect = true;
    ...
    function print(o) {
        if (jspmWSStatus()) {
            ...
            cmds += newLine;
            cmds += "{{ __('The best restaurant, and party center.') }}"; //text to print site title
            cmds += newLine;
            cmds += esc + '!' + '\x00';
            cmds += "---------------------------------------";                        
            cmds += newLine;
            ...
            cmds += newLine;
            cmds += "---------------------------------------";
            @if($order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered')
              @if(App\Models\Buffetorder::where('id', $order_id)->first()->client_id)
                cmds += newLine;
                cmds += esc + '!' + '\x08'; //Emphasized + Double-height + Double-width mode selected (ESC ! (8 + 16 + 32)) 56 dec => 38 hex
                cmds += "Customer Phone No: {{ $order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered' ? App\Models\Client::find(App\Models\Buffetorder::where('id', $order_id)->first()->client_id)->mobile : ''}}"               
                cmds += newLine;
                cmds += "Customer Order Table No: {{ $order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered' ? App\Models\Buffetsalebackup::where('buffetorder_id', $order_id)->first()->order_tbl_no : ''}}"               
              @else
                cmds += esc + '!' + '\x08'; 
                cmds += newLine;
                cmds += "Customer Order Table No: {{ $order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered' ? App\Models\Buffetsalebackup::where('buffetorder_id', $order_id)->first()->order_tbl_no : ''}}"               
            ...
            @php $paid_amount = App\Models\Buffetorder::find($order_id)->cash_pay + App\Models\Buffetorder::find($order_id)->card_pay +
            App\Models\Buffetorder::find($order_id)->mobile_banking_pay; 
            //calculating total discount amount received.
            $discount = App\Models\Buffetorder::find($order_id)->discount + App\Models\Buffetorder::find($order_id)->reward_discount +
            App\Models\Buffetorder::find($order_id)->card_discount + App\Models\Buffetorder::find($order_id)->gpstar_discount;            
            @endphp
            @foreach(App\Models\Buffetsalebackup::where('buffetorder_id', $order_id)->get() as $saleCart)        
            ...
            cmds += "                          -------------";
            cmds += newLine;
            cmds += '          Amount Due:          {{ $order_id ? round(App\Models\Buffetorder::find($order_id)->grand_total,2) : '' }}'; 
            cmds += newLine;           
            cmds += "                Paid:          {{ $order_id && App\Models\Buffetorder::where('id', $order_id)->first()->status == 'delivered' ? $paid_amount : '' }}";
            cmds += newLine; 
            cmds += 'Payment Mode:          ';
            cmds += newLine; 
            cmds += "---------------------------------------";
            cmds += newLine;
            @if($order_id)
            @if((float)App\Models\Buffetorder::find($order_id)->cash_pay)
            cmds += 'CASH:                          {{ $order_id ? round(App\Models\Buffetorder::find($order_id)->cash_pay,2) : '' }}';
            cmds += newLine;
            @endif
            @if((float)App\Models\Buffetorder::find($order_id)->card_pay)
            cmds += 'CARD:                          {{ $order_id ? round(App\Models\Buffetorder::find($order_id)->card_pay,2) : '' }}';
            cmds += newLine;
            @endif
            @if((float)App\Models\Buffetorder::find($order_id)->mobile_banking_pay)
            cmds += 'Mobile Banking:                {{ $order_id ? round(App\Models\Buffetorder::find($order_id)->mobile_banking_pay,2) : '' }}';
            cmds += newLine;
            @endif
            @endif
            cmds += "---------------------------------------";
            ...
            @endif
            cmds += newLine;
            @if((float)App\Models\Buffetorder::find($order_id)->fraction_discount)
            cmds += 'Other Discount:                {{ $order_id ? round(App\Models\Buffetorder::find($order_id)->fraction_discount,2) : '' }}';
            cmds += newLine;
            cmds += "---------------------------------------";
            cmds += newLine;
            @endif
            cmds += 'You Have Saved:                {{ $order_id ? round(($discount + App\Models\Buffetorder::find($order_id)->fraction_discount),2)  : '' }}';            
            @endif
