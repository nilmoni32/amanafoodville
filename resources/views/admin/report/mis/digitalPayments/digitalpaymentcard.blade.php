...
<div class="app-title">
    ...
    <div class="pull-right">
        <a href="{{ route('admin.reports.pdfgetDigitalPayment', [$start_date, $end_date, 'card']) }}" class="btn btn-sm btn-dark"
    ...
    <div class="col-md-12">
        <div class="tile">
            <div class="tile-body">
                        ...
                        <form action="{{ route('admin.reports.getdigitalPayments') }}" method="post"
                            ...
                            <div class="form-group mx-sm-3 mb-2">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="digitalPayOption" id="inlineRadio1" value="card" required {{$card ? 'checked' : ''}}>
                                    <label class="form-check-label font-weight-bold" for="inlineRadio1">Card Payments</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="digitalPayOption" id="inlineRadio2" value="mobile">
                ...
                @if($card_sales->count() > 0)
                @php $total_paid=0.0; $total_discount=0.0; @endphp                                             
                <table class="table table-hover table-bordered" id="sampleTable">
                    <thead>
                        <tr>
                            <th class="text-center"> # </th>                            
                            <th class="text-center"> Card Bank </th>
                            <th class="text-center"> Card Paid Amount</th>
                            <th class="text-center"> Card Discount Amount </th>
                        </tr>
                    </thead>
                    <tbody> 
                        @foreach($card_sales as $card)
                        @php $total_paid +=$card->card_paid; $total_discount +=$card->card_discount; @endphp               
                        <tr>
                            <td class="text-center">{{ $loop->index + 1}}</td>
                            <td class="text-center">{{ $card->bank_name }} </td>                        
                            <td class="text-center">{{ round($card->card_paid,2) }} {{ config('settings.currency_symbol') }}</td>
                            <td class="text-center">{{ round($card->card_discount,2) }} {{ config('settings.currency_symbol') }}</td>
                        </tr>
                        @endforeach
                        <tr>
                            <td colspan="6">
                                <div class="row h6">
                                    <div class="col-10 text-right">Total Card Paid :</div>
                                    <div class="col-2 pl-0"><span>{{ round($total_paid,2) }}</span><span class="ml-1 mr-2">{{ config('settings.currency_symbol') }}</span></div>                        
                                </div>
                                <div class="row h6">
                                    <div class="col-10 text-right">Total Card Discount :</div>
