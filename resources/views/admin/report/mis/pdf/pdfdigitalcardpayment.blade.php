...
<head>
    ...
    <title>MIS Report [Card Payment] - {{ config('app.name', 'Funville') }}</title>
    <style>
        body{
    ...
    <div class="content-wrapper">
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                <h6 style="margin-top:5px; text-align: left;">MIS Report:  Card Payment Details</h6>
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                ...
                @php $total_card_paid=0.0;  $total_card_discount=0.0; @endphp
                @foreach($card_sales as $sale)
                    @php 
                    $total_card_paid += $sale->card_paid; 
                    $total_card_discount += $sale->card_discount; 
                    @endphp
                @endforeach
                
                <p class="text-left" style="margin-top:5px;">
                    <div style="line-height: 20px;">
                        <span>Total Card Payments </span><span style="padding-left: 5px;">:</span> {{ round($total_card_paid,2) }} {{ config('settings.currency_symbol') }}
                    </div>
                    <div style="line-height: 20px;">
                        <span>Total Card Discount</span><span style="padding-left: 16px;">:</span> {{ round($total_card_discount,2) }} {{ config('settings.currency_symbol') }}
                    </div>                    
                </p>
            </div>
            <div class="clearfix"></div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $sale->bank_name }}</td>
                        <td class="text-center">{{ round($sale->card_paid,2)  }}
                            {{ config('settings.currency_symbol') }}</td>
                        <td class="text-center">{{ round($sale->card_discount,2)  }}
                            {{ config('settings.currency_symbol') }}</td>
                    </tr>
                    @endforeach
                </tbody>
