    ...
    <div class="content-wrapper">
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                <h6 style="margin-top:5px; text-align: left;">MIS Report:  Mobile Payment Details</h6>
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                ...
                @php $total_mobile_paid=0.0;  $total_mobile_discount=0.0; @endphp
                @foreach($mobile_sales as $sale)
                    @php 
                    $total_mobile_paid += $sale->mobile_paid; 
                    $total_mobile_discount += $sale->mobile_discount; 
                    @endphp
                @endforeach
                
                <p class="text-left" style="margin-top:5px;">
                    <div style="line-height: 20px;">
                        <span>Total Mobile Payments </span><span style="padding-left: 5px;">:</span> {{ round($total_mobile_paid,2) }} {{ config('settings.currency_symbol') }}
                    </div>
                    <div style="line-height: 20px;">
                        <span>Total Mobile Discount</span><span style="padding-left: 16px;">:</span> {{ round($total_mobile_discount,2) }} {{ config('settings.currency_symbol') }}
                    </div>                    
                </p>
            </div>
            <div class="clearfix"></div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ round($sale->mobile_paid,2)  }}
                            {{ config('settings.currency_symbol') }}</td>
                        <td class="text-center">{{ round($sale->mobile_discount,2)  }}
                            {{ config('settings.currency_symbol') }}</td>
                    </tr>
                    @endforeach
                </tbody>
