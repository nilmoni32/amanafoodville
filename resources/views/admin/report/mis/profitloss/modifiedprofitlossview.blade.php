...
<div class="app-title">
    ...
    <div class="pull-right">
        <a href="{{ route('admin.reports.pdfmodifiedgetprofitloss', [$start_date, $end_date]) }}" class="btn btn-sm btn-dark"
            ...
            <div class="tile-body">
                <div class="row mb-4">
                    <div class="col-12 pt-3">
                        <form action="{{ route('admin.reports.percentage.getprofitLoss') }}" method="post"
                ...
                @php $kot_totalSalesCost=0.0; $ecom_totalSalesCost=0.0; $buffet_totalSalesCost=0.0; 
                     $ecom_totalSales=0.0; $kot_totalSales=0.0; $buffet_totalSales=0.0;
                @endphp
                @foreach($kot_sales as $cart)
                    @php $kot_totalSalesCost += $cart->salesCost; $kot_totalSales += $cart->sales;
                    @endphp
                @endforeach 
                {{-- Calculating total sales of buffet  --}}
                @foreach($buffet_sales as $cart)
                    @php $buffet_totalSalesCost += $cart->salesCost; $buffet_totalSales += $cart->sales;
                    @endphp
                @endforeach 
