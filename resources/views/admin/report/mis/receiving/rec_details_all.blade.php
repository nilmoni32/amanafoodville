...
<div class="app-title">
    ...
    <div class="pull-right">
        <a href="{{ route('admin.reports.pdfReceiving', [$start_date, $end_date, $report_type, $supplier_id]) }}" class="btn btn-sm btn-dark"
    ...
    <div class="col-12">
        <div class="tile">
            <div class="tile-body px-3">
                ...
                <div class="row">
                    <div class="col-12">
                        <label class="control-label font-weight-bold">Supplier Name:  {{ App\Models\Supplier::find($challan->supplier_id)->name }}</label>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-2 col-12">
                        <label class="control-label font-weight-bold">Receiving No:  #{{ $challan->id }}</label>
                    </div>
                    <div class="col-md-3 col-12">
                        <label class="control-label font-weight-bold">Requisition Date: {{ explode(' ', $challan->requisition_date)[0] }}</label>
                    </div>
                    <div class="col-md-3 col-12">
                        <label class="control-label font-weight-bold">Receiving Date: {{ explode(' ', $challan->payment_date)[0] }}</label>
                    </div>
                    <div class="col-md-2 col-12">
                        <label class="control-label font-weight-bold">Total Qty: {{ $challan->total_quantity }}</label>
                    </div>
                    <div class="col-md-2 col-12">
                        <label class="control-label font-weight-bold">Total Amount: {{ round($challan->total_amount,2) }} {{ config('settings.currency_symbol') }}</label>
                    </div>
                </div>              
                ...
                <table class="table table-hover table-bordered mb-5" id="sampleTable">
                    ...
                    <tbody>
                        ...
                        @foreach(App\Models\ReceiveIngredientList::where('receive_from_supplier_id', $challan->id)->get() as $receiving_product)
                        <tr>
                            <td class="text-center">{{ $loop->index + 1  }}</td>
                            <td class="text-center">{{ $receiving_product->name }}</td>
                            <td class="text-center">{{ $receiving_product->unit }}</td>
                            <td class="text-center">{{ $receiving_product->stock }}</td>
                            <td class="text-center">{{ $receiving_product->quantity }}</td>
                            <td class="text-center">{{ $receiving_product->unit_cost }}</td>
                            <td class="text-center">{{ round($receiving_product->total,2) }} {{ config('settings.currency_symbol') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
