...
<head>
    ...
    <title>Requisition Details Report - {{ config('app.name', 'Funville') }}</title>
    <style>
        body{
    ...
    <div class="content-wrapper">
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                <h6 style="margin-top:5px; text-align: left;">Report: All Supplier Requisition Report Details</h6>                
            </div>
            <div class="clearfix"></div>
        </div>
        ...
        <div class="product-description" style="margin-top:-10px;">
            ...
            <table  style="margin-bottom:-15px;">
                <tbody>
                    <tr>
                        <td class="record text-center" colspan="5">Supplier: {{ App\Models\Supplier::find($requisition->supplier_id)->name }}</td>                        
                    </tr>
                    <tr>
                        <td class="record">Req. No:  #{{ $requisition->id }}</td>
                        <td class="record">Req. Date:  {{ date('d-m-Y', strtotime(explode(' ', $requisition->requisition_date)[0])) }}</td>
                        <td class="record">Exp. Delivery Date:  {{ date('d-m-Y', strtotime(explode(' ', $requisition->expected_delivery)[0])) }}</td>
                        <td class="record">Total Qty:  {{ $requisition->total_quantity }}</td>
                        <td class="record">Total Amount:  {{ round($requisition->total_amount,2) }} {{ config('settings.currency_symbol') }}</td>
                    </tr>
                </tbody>                
            </table>            
            <table>
