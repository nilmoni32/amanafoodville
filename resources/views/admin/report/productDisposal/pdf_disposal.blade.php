...
<head>
    ...
    <title>Disposal Report - {{ config('app.name', 'Funville') }}</title>
    <style>
        body{
    ...
    <div class="content-wrapper">
        ...
        <div class="invoice-description">
            <div class=" float-left" style="width:100%;">
                <h5 style="margin-top: 0px; padding-bottom:10px; text-align:left; border-bottom: .8pt solid #5e5e5e;">Product Disposal Invoice</h5>
            </div>           
            <div class="clearfix"></div>
        </div>
        <div class="invoice-description" style="margin-top:-18px;">
            <div class="float-left" style="width:100%;">
                <h6 style="margin:0; text-align:left;">Disposal Product Details:</h6>
            </div>            
            <p style="width:50%; padding-top:25px; float:left;"><span style="font-weight:bold;">Disposal No : </span>#{{ $product_disposal->id }} </p>            
            <p style="width:50%;  padding-top:25px; float:right; direction: rtl;"><span style="font-weight:bold;">Disposal Date: </span> {{  date('d-m-Y', strtotime(explode(' ', $product_disposal->created_at)[0])) }}</p>
            <div class="clearfix"></div>
        </div>
        <div class="invoice-description" style="margin-top:-25px;">
            <p style="width:50%;  float:left;"><span style="font-weight:bold;">Reason :</span> {{ $product_disposal->reason }}</p>            
            <p style="width:50%;  float:right; direction: rtl;"><span style="font-weight:bold;">Remarks: </span>{{ $product_disposal->remarks }}</p>
            <div class="clearfix"></div>
        </div>       
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="text-align:left; width:33%;"> Item Name </th>
                        <th style="text-align:right"> Stk Unit </th>
                        <th style="text-align:right"> Prev. Stk Qty </th>    
                        <th style="text-align:right"> Curr. Stk Qty </th>                    
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-left" style="width:33%;">{{ $item->supplier_stock->supplier_product_name }}</td>                        
                        <td style="text-align:right">{{ $item->unit }}</td>
                        <td style="text-align:right">{{ $item->stock }}</td>
                        <td style="text-align:right">{{ $item->supplier_stock->total_qty }}</td>
                        <td style="text-align:right; width:11%;">{{ $item->quantity }}</td>
                        <td style="text-align:right; width:13%;">{{ number_format($item->unit_cost , 2, '.', '') }} {{ config('settings.currency_symbol') }}</td>
                        <td style="text-align:right; width:14%;">{{ number_format($item->total , 2, '.', '') }} {{ config('settings.currency_symbol') }}</td>                 
                    </tr>
                    @endforeach
                    <tr>
        ...
        <div class="product-description" style="margin-top:25px;">
            <p style="width:100%;">
                <span style="border-bottom:0.8pt solid #000; width:25%;text-align:center;float:left; padding-bottom:3px;">
                    {{ $product_disposal->admin->name}}</span>
